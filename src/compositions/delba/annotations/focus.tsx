...
export const focus: AnnotationHandler = {
  ...
  PreWithRef: (props) => {
    ...
    useScrollToFocus(ref) // Ensure scrolling to focused lines
...
function useScrollToFocusOrig(ref: React.RefObject<HTMLPreElement>) {
    ...
    if (ref.current) {
      ...
      if (bottom > containerRect.height || top < 0) {
        ref.current.scrollTo({
          ...
          behavior: firstRender.current ? "smooth" : "smooth",
        })
      }
      if (focusedElements.length > 0) {
        const lastElement = focusedElements[focusedElements.length - 1]
