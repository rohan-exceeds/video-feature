...
function addBlockComments(changes: CodeChanges, oldCode: string, newCode: string): { beforeCode: string, afterCode: string } {
  ...
  addedBlocks.forEach(block => {
      const comment = `// !block(1:${block.count}) 0,3,teal`;
      const insertIndex = block.startLine - 1;
...
Overview:
- Original code length: ${changes.metrics.totalLinesOld} lines
- Modified code length: ${changes.metrics.totalLinesNew} lines
- Total changes detected: ${changes.metrics.totalChanges}
- Change percentage: ${changes.metrics.changePercentage}%

Change Breakdown:
- Added lines: ${changes.addedLines.length}
- Removed lines: ${changes.removedLines.length}
`;
}
...
const newCode = 'import { useFetchCodeCommits } from "@/hooks/useApi";\n'+
'import useAuth from "@/hooks/useAuth";\n'+
'import { areArraysSimilar } from "@/store/generalSlice";\n'+
'import {\n'+
'  setPackagingResult,\n'+
...
processCodeChanges(oldCode, newCode);